...
public class Orden_comprasBean {
  ...
  public void registrar() {
    ...
    try {
      ...
      Date fecha = this.orden_compras.getFecha_entrega();
      String fechaString = formateador.format(fecha);
      Date tranformada = formateador.parse(fechaString);

      orden_compras.setFecha_entrega((java.sql.Date) tranformada);

      fecha = this.orden_compras.getFecha_orden();
      fechaString = formateador.format(fecha);
      tranformada = formateador.parse(fechaString);
      orden_compras.setFecha_orden((java.sql.Date) tranformada);
